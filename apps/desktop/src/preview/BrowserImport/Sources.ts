 ...
 *
 * Falls back to the single default directory when the metadata is unreadable:
 * a browser that has only ever had one profile is the common case, and failing
 * the whole import over a missing display name would be disproportionate.
 */
export const listSourceProfiles = Effect.fn("BrowserImportSources.listSourceProfiles")(function* (
  ...
  const profiles = yield* fileSystem.readFileString(context.path.join(root, "Local State")).pipe(
    Effect.flatMap(decodeLocalState),
    Effect.map((state) =>
      Object.entries(state.profile?.info_cache ?? {}).map(([directory, info]) => ({
        directory,
