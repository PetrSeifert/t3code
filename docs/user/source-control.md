...
- When you publish over SSH and the selected key needs a passphrase or password, T3 Code asks for it in the publish flow. It is passed to the server's SSH process for that attempt and is not saved.
...
- **SSH publish keeps asking for a passphrase** – Add the key to an SSH agent on the machine running the T3 Code server if you want it remembered between publish attempts
