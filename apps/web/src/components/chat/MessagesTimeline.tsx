...
function WorkingTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "working" }> }) {
  return (
    <div>
      <div className="border-b border-border/60 pb-2 pt-1">
        <div className="px-1 text-sm leading-relaxed text-secondary-label tabular-nums">
          {row.createdAt ? (
            <>
...
function ThinkingActivityRow() {
  ...
  return (
    <LiveActivityRow label={workingStepLabel ? `Thinking · ${workingStepLabel}` : "Thinking"} />
  );
}
...
function LiveWorkEntryTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "work-live" }> }) {
  ...
  const failed = workEntryIndicatesToolFailure(row.entry);
...
const AgentSpawnCtaRow = memo(function AgentSpawnCtaRow(props: { workEntry: TimelineWorkEntry }) {
  ...
  return (
    <div className="@container/agent-group -mx-1 rounded-xl border border-border/70 bg-secondary p-1.5 dark:border-transparent dark:bg-input/32">
      <div className="flex items-center justify-between gap-2 px-1">
        <div className="flex min-w-0 flex-1 items-center gap-1.5 px-1 py-1 text-left text-xs leading-4">
          <span className="sr-only @[32rem]/agent-group:hidden">{status}</span>
          ...
          <span className="ml-auto hidden shrink-0 items-center gap-2 text-[11px] text-muted-foreground @[32rem]/agent-group:flex">
            <span>{status}</span>
            {totalTokens > 0 ? (
